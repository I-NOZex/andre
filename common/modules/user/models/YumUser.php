...
class YumUser extends YumActiveRecord
{
  ...
  public function getActiveMemberships()
  {
    ...
    Yii::import('common.modules.membership.models.*');
  ...
  public function can($action, $subaction = null) {
	...
	public function relations() {
		...
		if($relations === false) {
			...
			if (Yum::hasModule('message')) {
				Yii::import('common.modules.message.models.*');
			...
			if (Yum::hasModule('profile')) {
				Yii::import('common.modules.profile.models.*');
	...
	public function getFriends($everything = false)
	{
		...
		$friends = array();
		Yii::import('common.modules.friendship.models.YumFriendship');
				...
				if ($user->activationKey == urldecode($key)) {
					...
					if ($user->save(false, array('activationKey', 'status'))) {
						...
						if (Yum::hasModule('message')
							...
							Yii::import('common.modules.message.models.YumMessage');
