...
/**
 * Freshy2Menu displays a multi-level menu using nested HTML lists.
 *
 * The main property of Freshy2Menu is {@link items}, which specifies the possible items in the menu.
 ...
 *
 * The following example shows how to use Freshy2Menu:
 * <pre>
 * $this->widget('zii.widgets.Freshy2Menu', array(
 *     'items'=>array(
 ...
 * @author Jonah Turnquist <poppitypop@gmail.com>
 * @author Qiang Xue <qiang.xue@gmail.com>
 * @version $Id: Freshy2Menu.php 2326 2010-08-20 17:02:07Z qiang.xue $
 * @package zii.widgets
 * @since 1.1
 */
class ArtMenu extends CWidget
{
	/**
	 ...
	 */
	public $submenuHtmlOptions=array('class'=>'dropdown');
	/**
	...
	protected function renderMenu($items)
	{
		if(count($items))
		{
			echo CHtml::openTag('nav',array('class'=>'top-bar','data-topbar'));
			    echo CHtml::openTag('section',array('class'=>'top-bar-section'));
			        echo CHtml::openTag('ul',array('class'=>'right'));
	...
	protected function renderMenuRecursive($items)
	{
		...
		foreach($items as $item)
		{
            ...
            if(isset($item['items']) && count($item['items'])){
                $options['class'].=' has-dropdown not-click';
            }
			echo CHtml::openTag('li', $options);

			if(isset($item['url']))
			{
				$label=$this->linkLabelWrapper===null ? $item['label'] : '<'.$a_class .' '. $this->linkLabelWrapper.'>'.$item['label'].'</'.$this->linkLabelWrapper.'>';
